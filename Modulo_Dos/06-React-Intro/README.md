...
Las _props_ funcionan como los _atributos_ HTML, es decir, que cuando usamos un Componente podemos agregarle _props_ escribiendo su nombre dentro del tag del  mismo. Por ejemplo, para agregar la _prop_ `name` al Componente que habiamos creado antes, cuando lo usamos escribimos `<HelloWorld name='Toni' />`. De esta forma, podemos pasar una o varias _props_ al mismo Componente.
...
ReactDOM.render(<HelloWorld name='Mi Curso!' />, document.getElementById('app'));
```
...
React nos da una forma simple de acceder a ellos, y es con la propiedad `children` de `this.props`. En este ejemplo en particular, la propiedad `this.props.children` del Componente _Nombre_ va a tomar el valor 'Mi Curso'.
